...
int verifica_principalsWithMultipleCategories(TVSeriesAPP& app)
{
   int er = 0;
    ...
    res="Chris Greenwood, Dave Baines, David Foulkes, Di Evans, Donnovan Harris, Joseph Hassell, Laura Balfour, Matt Jackson, Matt Ward, ";
    if (str!=res)
    {
...
int parseTitlePrincipals(const string& filename,TVSeriesAPP& Tree) {
    ...
    while (getline(file, line)) {
        ...
        Tree.addTitlePrincipals(principal);
